...
struct WorkoutDayDetailsView: View {
        ...
        Section {
            HStack {
                ...
                if isCardio(exercise) {
                    cardioFields(for: set)
                } else {
                    strengthFields(for: set)
                }
            }
            .padding(.horizontal, 0)
            .frame(maxWidth: .infinity, alignment: .leading)
    ...
    // MARK: - Fields for Cardio
    
    private func cardioFields(for set: ExerciseSet) -> some View {
        HStack {
            // Для Distance добавляем суффикс "km"
            dataInputField(
    ...
    // MARK: - Fields for Strength
    private func strengthFields(for set: ExerciseSet) -> some View {
        HStack {
            dataInputField(
                placeholder: "Weight",
                value: Binding(
    ...
    // MARK: - Helper for Cardio Check
    
    private func isCardio(_ exercise: Exercise) -> Bool {
        let isCardio = exercise.categories?.name?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "cardio"
        return isCardio
    }
